  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full max-w-md mt-2">
      ...
      <div className="flex flex-col gap-3">
        <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" className={inputCls + " w-full"} />
        <button type="submit" disabled={loading} className={`h-11 w-full sm:w-auto px-5 rounded-lg text-sm font-semibold transition-all disabled:opacity-70 whitespace-nowrap flex items-center justify-center gap-2 active:scale-95 ${dark ? "bg-white hover:bg-indigo-50 text-indigo-700 shadow-lg" : "bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-500/20"}`}>
          {loading ? (
